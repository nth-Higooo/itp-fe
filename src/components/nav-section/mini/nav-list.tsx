...
export function NavList({ data, depth, render, cssVars, slotProps, enabledRootRedirect }: any) {
  ...
  // Hidden item by role
  if (data.children) {
    const requiredPermissions = data.children.map((c: any) => c.permission);
    const hasPermission = hasViewPermission(requiredPermissions);
    if (!hasPermission) {
      return null;
    }
  } else {
    const hasPermission = hasViewPermission([data.permission]);
    if (!hasPermission) {
      return null;
    }
  }
