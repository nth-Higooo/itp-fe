...
export function KanbanView() {
  ...
  const [columnFixed, setColumnFixed] = useState<any>(true);

  const recentlyMovedToNewContainer = useRef<any>(false);
    ...
    (args: any) => {
      if (activeId && activeId in board.tasks) {
        return closestCenter({
          ...
          droppableContainers: args.droppableContainers.filter(
            (column: any) => column.id in board.tasks
          ),
        });
      }
      ...
      if (overId != null) {
        if (overId in board.tasks) {
          const columnItems = board.tasks[overId].map((task: any) => task.id);
          ...
          if (columnItems.length > 0) {
            ...
            overId = closestCenter({
              ...args,
              droppableContainers: args.droppableContainers.filter(
                (column: any) => column.id !== overId && columnItems.includes(column.id)
              ),
            })[0]?.id;
          }
        }
  ...
  const findColumn = (id: any) => {
    if (id in board.tasks) {
      return id;
    }

    return Object.keys(board.tasks).find((key) =>
      board.tasks[key].map((task: any) => task.id).includes(id)
    );
  };
  ...
  const onDragOver = ({ active, over }: any) => {
    ...
    if (activeColumn !== overColumn) {
      const activeItems = board.tasks[activeColumn].map((task: any) => task.id);
      const overItems = board.tasks[overColumn].map((task: any) => task.id);
      ...
      const updateTasks = {
        ...board.tasks,
        [activeColumn]: board.tasks[activeColumn].filter((task: any) => task.id !== active.id),
        [overColumn]: [
  ...
  const onDragEnd = ({ active, over }: any) => {
    ...
    if (overColumn) {
      const activeContainerTaskIds = board.tasks[activeColumn].map((task: any) => task.id);
      const overContainerTaskIds = board.tasks[overColumn].map((task: any) => task.id);
            ...
            >
              {board?.columns.map((column: any) => (
                <KanbanColumn key={column.id} column={column} tasks={board.tasks[column.id]}>
                  ...
                  >
                    {board.tasks[column.id].map((task: any) => (
                      <KanbanTaskItem
                        task={task}
                        key={task.id}
                        columnId={column.id}
