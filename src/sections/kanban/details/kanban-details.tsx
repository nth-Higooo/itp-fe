...
const StyledLabel = styled('span')(({ theme }: { theme: TTheme }) => ({
...
export function KanbanDetails({
  ...
  const handleChangeTaskName = useCallback((event: any) => {
  ...
  const handleChangeTaskDescription = useCallback((event: any) => {
    setTaskDescription(event.target.value);
  }, []);

  const handleChangePriority = useCallback((newValue: any) => {
    setPriority(newValue);
  }, []);

  const handleClickSubtaskComplete = (taskId: any) => {
        ...
        {!!task.labels.length && (
          <Box sx={{ gap: 1, display: 'flex', flexWrap: 'wrap' }}>
            {task.labels.map((label: any) => (
              <Chip key={label} color="info" label={label} size="small" variant="filled" />
            ))}
          </Box>
        )}
      </Box>
