...
// ----------------------------------------------------------------------
type TKanbanContactsDialogProps = any;

export function KanbanContactsDialog({ assignee = [], open, onClose }: TKanbanContactsDialogProps) {
  const [searchContact, setSearchContact] = useState('');

  const handleSearchContacts = useCallback((event: any) => {
          ...
          <Scrollbar sx={{ height: ITEM_HEIGHT * 6, px: 2.5 }}>
            <Box component="ul">
              {dataFiltered.map((contact: any) => {
                const checked = assignee.map((person: any) => person.name).includes(contact.name);
...
function applyFilter({ inputData, query }: any) {
  if (query) {
