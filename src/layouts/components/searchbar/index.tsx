import { useState, useCallback, ChangeEvent, KeyboardEvent, Fragment } from 'react';
...
import { hasViewPermission } from 'src/auth/context';
    ...
    return Object.keys(dataGroups)
      ...
      .map((group, index) => (
        <Box component="ul" key={`${group}-${index}`}>
          {dataGroups[group].map((item: any, cindex: number) => {
            const { title, path, permission, searchable } = item;

            const partsTitle = parse(title, match(title, searchQuery));

            const partsPath = parse(path, match(path, searchQuery));

            const hasPermission = hasViewPermission([permission]);

            return hasPermission && searchable ? (
              <Box
                component="li"
                key={`${group}-${index}-${title}${path}${cindex}`}
                sx={{ display: 'flex' }}
              >
                <ResultItem
                  path={partsPath}
              ...
              <Fragment key={`${group}-${index}-${title}${path}${cindex}`}></Fragment>
            );
          })}
        </Box>
      ));
